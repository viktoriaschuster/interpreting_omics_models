...
def run_nb_model(y_pos, y_neg, gene_idx, conditions, pairing):
    ...
    glm_model = sm.GLM(gene_expression, X, family=NegativeBinomial())
...
print(f"There are {len(go_ids_filtered_bp)} go terms that are {go_category} terms")
...
"""
# cant just predict from the model, so I use the original data
y = np.asarray(data.X.todense())
...
for i in pbar:
    ...
    # save the data
    if small_features:
        file_name = f'03_results/reports/sc_enformer_sae_go_analysis_small_{go_category}.csv'
    else:
        file_name = f'03_results/reports/sc_enformer_sae_go_analysis_large_{go_category}.csv'
...
if small_features:
    np.save(f'03_results/reports/sc_enformer_sae_gene_hits_per_feature_small_{go_category}.npy', gene_hits_per_feature)
else:
    np.save(f'03_results/reports/sc_enformer_sae_gene_hits_per_feature_large_{go_category}.npy', gene_hits_per_feature)
