...
verbose = False
print(f"Using device: {device}, seed: {seed}, go_category: {go_category}")
...
if seed == 0:
    ...
    sae_model_save_name = f'03_results/models/sae_model_10000_l1-1e-3_lr-1e-4_500epochs_seed{seed}'
...
if seed == 0:
    ...
    print(f"Loading activations for seed {seed}")
    batch_size = 128
    activations = []
...
print(f"Found {len(active_feature_ids)} active features")
...
nonzero_feats = torch.Tensor(list(
    set(torch.where(n_zeros_per_feature > activations.shape[0] - 100)[0].numpy()).union(
    set(torch.where(n_zeros_per_feature < (activations.shape[0] - 100))[0].numpy()))
))
...
logical_cores = psutil.cpu_count(logical=True) # This is what mp.cpu_count() usually returns
...
if verbose:
    ...
    print(f"Available Memory (at script start): {available_memory_gb:.2f} GB")
...
gene_name_to_ref_idx_map = {name: i for i, name in enumerate(adata_go.var['name'])}
pool = mp.Pool(mp.cpu_count())
...
for i in pbar:
    ...
    gene_p_values = DEG_analysis_unpaired(y_pos, y_neg, data_gene_names, pool)
    ...
    # save the data
    #if small_features:
    #    file_name = f'03_results/reports/sc_dgd_sae_go_analysis_small_2_{go_category}_rs{seed}.csv'
    #else:
    #    file_name = f'03_results/reports/sc_dgd_sae_go_analysis_large_2_{go_category}_rs{seed}.csv'
    #if not os.path.exists(file_name):
    #    results_df.to_csv(file_name, index=False)
    #else:
    #    results_df.to_csv(file_name, mode='a', header=False, index=False)
    #print(f"Finished: Total time: {end_time - start_time:.4f} seconds.")
    #break
...
file_name = f'03_results/reports/sc_dgd_sae_go_analysis_{go_category}_rs{seed}.csv'
results_df.to_csv(file_name, index=False)
np.save(f'03_results/reports/sc_dgd_sae_gene_hits_per_feature_{go_category}_rs{seed}.npy', gene_hits_per_feature)

# save the gene hits per feature
#if small_features:
#    np.save(f'03_results/reports/sc_dgd_sae_gene_hits_per_feature_small_2_{go_category}_rs{seed}.npy', gene_hits_per_feature)
#else:
#    np.save(f'03_results/reports/sc_dgd_sae_gene_hits_per_feature_large_2_{go_category}_rs{seed}.npy', gene_hits_per_feature)
