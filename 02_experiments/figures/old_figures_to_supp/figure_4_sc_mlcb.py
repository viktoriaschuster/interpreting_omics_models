...
fig, (axs, cax) = plt.subplots(2, 5, figsize=(18, 4), gridspec_kw={'height_ratios':[1,0.1]})
...
axs[0].text(-0.2, 1.1, "A", fontsize=18, transform=axs[0].transAxes, fontweight='bold')
...
sns.scatterplot(data=df_celltypes.sort_values(by='feat2306', ascending=True), x="PC 1", y="PC 2", hue="feat2306", s=1, alpha=0.7, ec=None, ax=axs[0], palette=feat_palette)
axs[0].set_title("Global feature:\nNeuron 2306")
# remove the ticks
axs[0].set_xticks([])
axs[0].set_yticks([])
cax[0].axis('off')
#cax[0].set_visible(False)
...
# plot the cbar underneath the plot
#from mpl_toolkits.axes_grid1 import make_axes_locatable
#divider = make_axes_locatable(axs[0])
#cbar_ax = divider.append_axes("bottom", size="5%", pad=0.1)
#cbar_points = plt.colorbar(cbar_points, cax=cbar_ax, label='Activation', orientation='horizontal', aspect=50)
#fig.add_axes(cbar_ax)
fig.colorbar(cbar_points, ax=cax[0], orientation='horizontal', pad=-0.2, aspect=50)
# remove everything else from the cax (xticks, yticks, etc.)
#plt.colorbar(cbar_points, ax=axs[0], label='Activation', orientation='horizontal', pad=0.1, aspect=50)
#plt.colorbar(cbar_points, ax=axs[0], label='Activation', orientation='horizontal', pad=0.1, aspect=50, shrink=0.5)
# remove the normal legend
axs[0].get_legend().remove()
# move the cbar legend to the bottom
#axs[0].legend(title='Activation', loc='upper left', bbox_to_anchor=(1, 1), frameon=False, markerscale=marker_scale, handletextpad=handletextpad, alignment='left')

# proerythroblast perturbation
feature = 2306
axs[1].text(-0.2, 1.1, "B", fontsize=18, transform=axs[1].transAxes, fontweight='bold')
i = 1
ct1 = 'Proerythroblast'
ct2 = 'Normoblast'
...
perturbation_palette = {'other': 'lightgrey', 'normal': 'black', 'perturbed': 'cornflowerblue'}
sns.scatterplot(data=df_temp, x='PC 1', y='PC 2', hue='type', size='size', ax=axs[i], palette=perturbation_palette, ec=edgecolor, s=point_size, alpha=0.3, sizes=(5,1))
axs[i].set_xticks([])
axs[i].set_yticks([])
axs[i].set_title('Steering\nNeuron 2306')
# legend to the right
# remove the first handle and label
handles, labels = axs[i].get_legend_handles_labels()
for handle in handles:
    handle.set_alpha(1)
axs[i].legend(handles=handles[2:-3], labels=labels[2:-3], loc='center left', ncol=2, bbox_to_anchor=(-0.1, -0.3), fontsize=12, frameon=False, markerscale=marker_scale, handletextpad=handletextpad, alignment='left', columnspacing=columnspacing)#.remove()

###
# second row: local features
###
i = 2
axs[i].text(-0.2, 1.1, "C", fontsize=18, transform=axs[i].transAxes, fontweight='bold')
#feature = 1238
feature = 1500
df_celltypes[str(feature)] = activations[:, feature].detach().cpu().numpy()
feat_palette = sns.color_palette('rocket_r', as_cmap=True)
# the second plot will be the same latent space but colored by the activation of feature 2306
sns.scatterplot(data=df_celltypes.sort_values(by=str(feature), ascending=True), x="PC 1", y="PC 2", hue=str(feature), size=str(feature), sizes=(1,10), alpha=0.7, ec=None, ax=axs[i], palette=feat_palette)
axs[i].set_title("Local feature:\nNeuron {}".format(feature))
# remove the ticks
axs[i].set_xticks([])
axs[i].set_yticks([])
# put the leend outside the plot
...
# the second plot will be the same latent space but colored by the activation of feature 2306
sns.scatterplot(data=df_celltypes.sort_values(by=str(feature), ascending=True), x="PC 1", y="PC 2", hue=str(feature), size=str(feature), sizes=(1,10), alpha=0.7, ec=None, ax=axs[i], palette=feat_palette)
axs[i].set_title("Neuron {}".format(feature))
# remove the ticks
axs[i].set_xticks([])
axs[i].set_yticks([])
# put the leend outside the plot
...
#plt.tight_layout()
# save
plt.savefig("03_results/figures/sc_reps_all_v4.png", dpi=300, bbox_inches='tight')