...
#handles_n = ['hidden variable'] + handles[2:8] + ['latent'] + handles[8:]
#labels_n = [''] + labels[2:7] + [''] + [''] + labels[8:]
#ax.legend(handles_n, labels_n, loc='upper left', bbox_to_anchor=(1.0, 1.0), markerscale=2, handletextpad=0.01, frameon=False)
ax.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0), markerscale=1.5, frameon=False, handletextpad=0.2, alignment='left', columnspacing=0.8, labelspacing=0.2)
...
ax2.text(-0.15, 1.05, "B", fontsize=18, transform=ax2.transAxes, fontweight='bold')
...
plt.savefig('03_results/figures/simL_variable_corr_with_baseline.pdf', bbox_inches='tight')