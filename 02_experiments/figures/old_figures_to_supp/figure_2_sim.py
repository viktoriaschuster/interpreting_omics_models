...
df_sae_metrics = pd.read_csv('03_results/reports/files/sim_modl1l4_sae-vanilla_metrics.csv')
...
#df_sae_metrics = df_sae_metrics[df_sae_metrics['lr'] == 1e-5]
...
df_corr_metrics = df_sae_metrics[['hidden_factor','type','n_activation_features']].copy() # take it out now before setting the categories
...
ax0.text(ax_text_x, ax_text_y, "A", fontsize=16, transform=ax0.transAxes, fontweight='bold')
...
ax1.text(ax_text_x, ax_text_y, "B", fontsize=16, transform=ax1.transAxes, fontweight='bold')
# the first plot is the loss vs hidden colored by model type
x = 'lr'
y = 'loss'
y_label = 'MSE loss'
sns.lineplot(data=df_lr, x=x, y=y, ax=ax1, hue='type', style='type', markers=True, palette=model_palette)
ax1.set_xscale('log')
ax1.set_yscale('log')
...
ax2.text(ax_text_x+0.05, ax_text_y, "C", fontsize=16, transform=ax2.transAxes, fontweight='bold')
# plot the correlation of X vs hidden factor
x = 'n_activation_features'
y = 'highest_corrs_tf'
y_label = 'max(corr(neurons, X))'
sns.lineplot(data=df_sae_metrics[df_sae_metrics['lr'] == 1e-5], x=x, y=y, ax=ax2, hue='type', style='type', markers=True, palette=model_palette)
ax2.set_xscale('log')
ax2.set_title(y_label)
ax2.set_xlabel('SAE hidden dimensionality')
ax2.set_ylabel('Pearson correlation')
ax2.legend(loc='center left', bbox_to_anchor=(0.35, 0.25), ncol=1, frameon=False, title='SAE type', handletextpad=0.2, alignment='left', columnspacing=0.8, labelspacing=0.2)

"""
# this will go to the supps
...
ax3.text(ax_text_x-0.1, ax_text_y, "D", fontsize=16, transform=ax3.transAxes, fontweight='bold')
x = 'n_activation_features'
y = 'correlation'
y_label = 'Pearson correlation'
hue = 'target'
sns.lineplot(data=df_corr_metrics, x=x, y=y, ax=ax3, hue=hue, style=hue, markers=True, palette=variable_palette)
ax3.set_xscale('log')
ax3.set_xlabel('SAE hidden dimensionality')
ax3.set_ylabel('Pearson correlation')
ax3.set_title('max(corr(neurons, variables))')
ax3.legend(loc='upper left', bbox_to_anchor=(0.48, 0.77), ncol=1, frameon=False, title='Variable', handletextpad=0.2, alignment='left', columnspacing=0.8, labelspacing=0.2)


plt.show()
# export to pdf
fig.savefig('03_results/figures/fig2_sim_modl1l4_sae_performance_metrics.pdf', bbox_inches='tight')

####################################################################################################

#plt.show()
# export to pdf
#fig.savefig('03_results/figures/fig2_sim_modl1l4_sae_recovery_metrics_a.pdf', bbox_inches='tight')
...
plt.show()
# export to pdf
fig.savefig('03_results/figures/fig2_sim_modl1l4_sae_recovery_metrics_b.pdf', bbox_inches='tight')

# print the rows where highest corr is less than 0.5
#print(df_sae_metrics[df_sae_metrics['highest_corrs_tf'] < 0.5].iloc[:,6:-5])
#print(df_sae_metrics.columns)
"""