...
def run_sae_analysis(activations, comparison_data):
    ...
    if len(unique_active_unit_indices) > 0:
        ...
        n_per_attribute = get_n_features_per_attribute(correlations_p)
        ...
        highest_corrs = get_highest_corr_per_attribute(correlations_p)
...
model_name = 'layer1_latent4_seed5'
latent_size = 4
...
for hidden_factor in hidden_factor_options:
    print('Hidden factor:', hidden_factor)
    for lr in tqdm(lr_options):
        for l1_weight in l1_weight_options:
            sae_setup = [hidden_factor, lr, l1_weight, sparsity_penalty]
            sae_model, loss = train_sae(train_loader, input_size=input_size, hidden_factor=hidden_factor, lr=lr, l1_weight=l1_weight, sparsity_penalty=sparsity_penalty, n_epochs=epochs[lr], sae_type=sae_type)
...
df_sae_metrics.to_csv('03_results/reports/files/sim_modl1l4_sae-'+sae_type+'2_metrics.csv', index=False)