...
encoder = Encoder2(20000, latent_dim, model_depth, dropout=hyperparams['dropout'], width=model_width).to(device)
decoder = Decoder2(latent_dim, 20000, model_depth, hidden_dims=encoder.hidden_dims, dropout=hyperparams['dropout'], width=model_width).to(device)
...
encoder, decoder, history = train_and_eval_model(
    ...
    n_epochs=n_epochs, batch_size=hyperparams['batch_size'], learning_rate=hyperparams['learning_rate'], weight_decay=hyperparams['weight_decay'],
...
import os
if not os.path.exists('03_results/models/'+run_name):
    os.makedirs('03_results/models/'+run_name)
torch.save(encoder, '03_results/models/'+run_name+'/encoder.pth')
torch.save(decoder, '03_results/models/'+run_name+'/decoder.pth')
# save the history as a csv
df_history = pd.DataFrame(history)
df_history.to_csv('03_results/models/'+run_name+'/history.csv')
...
# log the number of features in wandb
#wandb.run.summary['n_features'] = n_features
#wandb.run.summary['r_square_threshold'] = thresholds
for n, t in zip(n_features, thresholds):
    wandb.log({'n_features': n, 'r_square_threshold': t})
#wandb.log({'n_features': n_features, 'r_square_threshold': thresholds})
...
wandb.run.summary.update(r_squares)
#wandb.log(r_squares)
