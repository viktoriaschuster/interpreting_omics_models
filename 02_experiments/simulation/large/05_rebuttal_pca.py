...
def analyze_dimreduction_methods(latent, comparison_data, active_features=None, redundant=False, device='cpu'):
    if redundant:
    ...
    if active_features is None:
        active_features = np.arange(latent.shape[1])
    corrs = get_correlations_with_data(latent, active_features, comparison_data, device=device)
...
# filter out the components that explain no variance
#embed = embed[:, np.where(pca.explained_variance_ > 0)[0]]
#print("kept {} components".format(embed.shape[1]))
#print("\n")

print("Running y")
y_metrics = analyze_dimreduction_methods(embed, rna_counts, redundant=True, device=device)
#y_metrics = [None, np.ones(10), np.ones(10)]  # set to None to avoid saving the metrics
gc.collect()
torch.cuda.empty_cache()
print("Running x0")
...
df_pca.to_csv(f'03_results/reports/files/sim2L_pca_metrics_{n_dim}components.csv', index=False)