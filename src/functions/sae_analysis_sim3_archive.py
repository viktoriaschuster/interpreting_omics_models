...
def get_number_of_redundant_features(activations, unique_activs, threshold=0.95):
    ...
    redundant_set = set()
    for i in tqdm.tqdm(range(len(unique_activs))):
        i_corr = pearsonr(activations[:,unique_activs].cpu().detach(), activations[:, unique_activs[i]].cpu().detach())
        redundant_set.update([unique_activs[j] for j in np.where(i_corr.numpy() > threshold)[0]])
    n_redundant = len(redundant_set)
    return n_redundant

def get_correlations_with_data(activations, unique_activs, comparison_data):
    correlations_p = torch.zeros((len(unique_activs), comparison_data.shape[1]))#.to(activations.device)
    #correlations_s = np.zeros((len(unique_activs), comparison_data.shape[1]))
    
    #pool = mp.Pool(mp.cpu_count())
    #correlations_p_hstack = pool.starmap(pearsonr, [(activations[:,unique_activs].cpu().detach(), comparison_data[:, i]) for i in range(comparison_data.shape[1])])
    #pool.close()
    #correlations_p = torch.stack(correlations_p_hstack, dim=1)
    
    comparison_data = comparison_data#.to(activations.device)
    for i in range(comparison_data.shape[1]): # way faster than mp
        # get the activations
        #correlations_p[:, i] = pearsonr(activations[:,unique_activs].cpu().detach(), comparison_data[:, i])
        correlations_p[:, i] = pearsonr(activations[:,unique_activs], comparison_data[:, i].to(activations.device)).detach().cpu()
    #return correlations_p.detach().cpu().numpy(), None
    return correlations_p.numpy(), None

def get_n_features_per_attribute(correlations, threshold=0.95):
    n_per_attribute = np.sum(np.abs(correlations) > threshold, axis=0)
...
def get_highest_corr_per_attribute(correlations):
    best_corrs = correlations[np.argmax(np.abs(correlations), axis=0), np.arange(correlations.shape[1])]
...
def run_sae_analysis(sae_model, train_loader, activations, comparison_data, corrtype='pearson', get_all=True):
    ...
    if len(unique_active_unit_indices) > 0:
        ...
        #print('Computing number of features per attribute')
        n_per_attribute = get_n_features_per_attribute(corrs)
        #print('Computing highest correlation per attribute')
